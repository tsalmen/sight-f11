...
<html xmlns="http://www.w3.org/1999/xhtml" dir="<?php bloginfo('text_direction'); ?>" xml:lang="<?php bloginfo('language'); ?>">
	...
	<body <?php 
		...
		/* Old code using ternary operator
		
		echo (get_option('bg_color')) ? 'style="background-color: '.get_option('bg_color'). '; background-image: url(\'' .get_option('bg_image'). '\');"' : '' 
		
		*/
		
		if(get_option('bg_color') || get_option('bg_image')) 
			{ 	echo 'style="background-color:'.get_option('bg_color') .
				';background-image:url(\'' .get_option('bg_image').'\');' . 
